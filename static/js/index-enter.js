...
$(function(){
    ...
    // 企业页面确认
    $('input[name="ceo"]').on('input propertychange',function(){
        if($('input[name="enterprise"]').val() == '' || $('input[name="enterprise"]').val() ==undefined){
            alert("请输入企业名称")
        }
        if($('input[name="enterprise"]').val() != '' && $('input[name="enterprise"]').val() !=undefined && $('input[name="ceo"]').val() != '' && $('input[name="ceo"]').val() !=undefined){
            $(".enterprise-button input").removeClass("submitError");
            $(".enterprise-button input").addClass("submitUser");
        }else{
            $(".enterprise-button input").removeClass("submitUser");
            $(".enterprise-button input").addClass("submitError");
        }
    });
})
//获取短信验证码
...
function stepThird(){
    if(ok3 == true){
        ...
        $(".ball-four").removeClass("ball-white").addClass("ball-blue");
    }
};
...
function stepTour(){
    if($('input[name="enterprise"]').val() != '' && $('input[name="enterprise"]').val() !=undefined && $('input[name="ceo"]').val() != '' && $('input[name="ceo"]').val() !=undefined){
        ...
        var vm={};
        vm.registerType="企业注册"
        vm.phone=$('input[name="phone"]').val();
        vm.username=$('input[name="user"]').val();
        vm.password=$('input[name="passward"]').val();
        vm.rePassword=$('input[name="repass"]').val();
        vm.employeeNumber=$('input[name="ceo"]').val();
        vm.enterpriseName=$('input[name="enterprise"]').val();
        vm.authCode=$('input[name="verification"]').val();
