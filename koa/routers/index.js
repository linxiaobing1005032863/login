...
module.exports = function(){
    ...
    router.get('/login', function *(){
        ...
        yield (sendfile(this, path.resolve('static/register.html')));
        if(!this.status){
            this.throw(404);
        }
    }).get('/register-enter', function *(){
        yield (sendfile(this, path.resolve('static/register-enter.html')));
        if(!this.status){
            this.throw(404);
        }
    }).get('/repass', function *(){
        yield (sendfile(this, path.resolve('static/repass.html')));
        if(!this.status){
            this.throw(404);
        }
    }).get('/market', function *(){
        ...
        yield (server().userlogin(user)
            ...
            var responseText = JSON.parse(parsedBody);
                if(responseText.code == 0){
                    ...
                    $self.body = responseText;
                    $self.cookies.set('token', token, {maxAge : 1000 * 60 * 60 * 24 * 7,domain:".issp.bjike.com"});
        ...
        yield (server().logout(token)
            .then((parsedBody) =>{
                var responseText = JSON.parse(parsedBody);
                if(responseText.code== 0 ){
                    $self.cookies.set('token', '', {domain:".issp.bjike.com"});
                    $self.cookies.set('username', '', {domain:".issp.bjike.com"});
             ...
             //获取验证码
    }).post('/user/verification', function*(){
        var $self = this;
        var user = $self.request.body;
        yield (server().userVerification(user)
            .then((parsedBody) =>{
                var responseText = JSON.parse(parsedBody);
                $self.body = responseText;
            }).catch((error) =>{
        ...
        var $self = this;
        var user = $self.request.body;
        yield (server().userRegister(user)
            .then((parsedBody) =>{
                var responseText = JSON.parse(parsedBody);
                $self.body = responseText;
            }).catch((error) =>{
        ...
        //提交注册（企业）
    }).post('/user/enterprise', function*(){
        var $self = this;
        var user = $self.request.body;
        yield (server().userEnterprise(user)
            .then((parsedBody) =>{
                var responseText = JSON.parse(parsedBody);
                $self.body = responseText;
            }).catch((error) =>{
    ...
    }).post('/user/repass', function*(){
        var $self = this;
        var user = $self.request.body;
        yield (server().userRepass(user)
            .then((parsedBody) =>{
                var responseText = JSON.parse(parsedBody);
                $self.body = responseText;
            }).catch((error) =>{
    ...
    }).post('/user/Verify', function*(){
        var $self = this;
        var user = $self.request.body;
        yield (server().userVerify(user)
            .then((parsedBody) =>{
                var responseText = JSON.parse(parsedBody);
                $self.body = responseText;
            }).catch((error) =>{
